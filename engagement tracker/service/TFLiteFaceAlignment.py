...
class BaseTFLiteFaceAlignment():
    ...
    def get_landmarks(self, image, detected_faces=None):
        ...
        #     if not detected_faces[0][2] - detected_faces[0][0] < 100:
        #         inp, M = self._preprocessing(image, detected_faces[0])
        #         self._inference(inp)
        ...
        if len(detected_faces) > 0:
            big_face = detected_faces[0]

            # finds the largest face in the image
            for box in detected_faces:
                if max(calculate_diagonal(big_face), calculate_diagonal(box)) == calculate_diagonal(box):
                    big_face = box

            if not big_face[2] - big_face[0] < 100:
                inp, M = self._preprocessing(image, big_face)
                self._inference(inp)

                yield self._postprocessing(M)


# returns the diagonal of the face box     
def calculate_diagonal(box):
    return pow(box[2] - box[0], 2) + pow(box[1] - box[3], 2)
